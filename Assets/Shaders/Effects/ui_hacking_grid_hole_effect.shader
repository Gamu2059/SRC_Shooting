// UIの特定の座標からグリッド状に穴が空いたように見せるシェーダー
Shader "Custom/UI/HackingGridHole"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                float x = floor(IN.texcoord.x * _GridNumX);
                // 分割数よりもインデックスが大きければ、1引く
                x -= step(x, _GridNumX);

                float y = floor(IN.texcoord.y * _GridNumY);
                // 分割数よりもインデックスが大きければ、1引く
                y -= step(y, _GridNumY);
