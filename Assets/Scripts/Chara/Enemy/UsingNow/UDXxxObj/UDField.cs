...
public class UDField : ScriptableObject
{
    ...
    public string m_EUDExp;

    [SerializeField, Tooltip("パラメータ")]
    public ExpFields m_ExpFields;
    ...
    public void Reset()
    {
        foreach (E_U_D eUD in System.Enum.GetValues(typeof(E_U_D)))
        {
            m_EUDExp += eUD;
            m_EUDExp += "\n\n";
            m_EUDExp += EUDToExpObject(eUD).GetExp();
            m_EUDExp += "\n\n\n";
        }
    }
    ...
    {
        m_EUDExp = m_EUD + "\n\n" + EUDToExpObject(m_EUD).GetExp();

        ExpAbstract expAbstract = EUDToExpObject(m_EUD);
