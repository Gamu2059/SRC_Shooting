...
public class IeaouKinkakuji : BattleRealEnemyController
{
    ...
    private void ShotOrNot()
    {
        ...
        if (m_ShotTime < 0f)
        {
            ...
            for (int i = 0; i < way; i++)
            {
                ...
                // 発射地点の領域を楕円にする
                if (0 < pastPhase && pastPhase < Mathf.PI)
                {
                    randomPos.x *= 1 - Mathf.Sin(pastPhase);
                }
                else
                {
                    randomPos.y *= -(1 - Mathf.Sin(pastPhase));
                }
