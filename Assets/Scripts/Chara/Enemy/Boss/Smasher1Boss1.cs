...
public class Smasher1Boss1 : System.Object
{
    ...
    public void Awakes()
    {
        m_DanmakuCountAbstractArrayArray = new DanmakuCountAbstract[m_UDParamsArrayArray.Length][];

        for (int i = 0;i < m_UDParamsArrayArray.Length;i++)
        {
            m_DanmakuCountAbstractArrayArray[i] = new DanmakuCountAbstract[m_UDParamsArrayArray[i].m_UDParametersArray.Length];

            for (int j = 0;j < m_UDParamsArrayArray[i].m_UDParametersArray.Length;j++)
            {
                m_DanmakuCountAbstractArrayArray[i][j] = EUDToUDObject(m_UDParamsArrayArray[i].m_UDParametersArray[j].m_EUD);

                m_DanmakuCountAbstractArrayArray[i][j].Awakes(m_UDParamsArrayArray[i].m_UDParametersArray[j]);
            }
        }
    }
    ...
    public void Updates(EnemyController enemyController)
    {
        ...
        else
        {
            ...
            {
                foreach (DanmakuCountAbstract danmakuCountAbstract in danmakuCountAbstractArray)
                {
